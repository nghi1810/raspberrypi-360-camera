...
def process_right(frame_right):
    K_right = np.array([
    [362.79422772,   0.,          351.93581808],
    [0.,           430.68859704, 247.57851458],
    [0.,             0.,           1.]
    ], dtype=np.float64)
...
if __name__ == "__main__":
    ...
    # Start processes (daemon)
    p  = Process(target=process_worker, args=(proc_in,  proc_out, process_front), daemon=True)
    p1 = Process(target=process_worker, args=(proc_in1, proc_out1, process_right), daemon=True)
    p2 = Process(target=process_worker, args=(proc_in2,  proc_out2, process_left), daemon=True)
    p3 = Process(target=process_worker, args=(proc_in3, proc_out3, process_back), daemon=True)

    p.start(); p1.start(); p2.start(); p3.start()
    ...
    while True:
        try:
            frame  = thread_queue_front.get_nowait()
            frame1 = thread_queue_right.get_nowait()
            frame2 = thread_queue_left.get_nowait()
            frame3 = thread_queue_back.get_nowait()
        except Empty:
            # one of the queues didn't have data -> skip this loop
            pass
        else:
            # Only runs if ALL 4 were successfully retrieved
            if not proc_in.full():
                proc_in.put(frame)
        ...
        try:
            transformed_frame_front = proc_out.get_nowait()
            transformed_frame_right = proc_out1.get_nowait()
            transformed_frame_left  = proc_out2.get_nowait()
            transformed_frame_back  = proc_out3.get_nowait()
        except Empty:
            # If any queue has no frame, skip this iteration
            pass
        else:
            # Only runs when ALL 4 frames were successfully retrieved
